...
from myshop.models import SmartCard, SmartPhoneModel, ScooterModel
...
class ScooterSerializer(ProductSerializer):
    class Meta:
        model = ScooterModel
        fields = ['product_name', 'slug', 'color']
...
class AddScooterToCartSerializer(AddToCartSerializer):
    """
    Modified AddToCartSerializer which handles Scooters
    """
    def get_instance(self, context, data, extra_args):
        product = context['product']
        if data is empty:
        ...
        try:
            ...
            variant = product.variants.first()
        extra.update(color=variant.color)
        instance = {
            'product': product.id,
            'product_code': variant.product_code,
            'unit_price': variant.unit_price,
