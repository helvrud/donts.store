...
from myshop.models import KhimageVariant, KhimageModel, ScooterVariant, ScooterModel
from myshop.models.i18n_polymorphic.smartphone import OperatingSystem
#~ from myshop.models.i18n_polymorphic.scooter import OperatingSystem
...
class KhimageInline(admin.TabularInline):
    model = KhimageVariant
    extra = 0


@admin.register(KhimageModel)
class KhimageAdmin(InvalidateProductCacheMixin, SortableAdminMixin, TranslatableAdmin, FrontendEditableAdminMixin,
                      CMSPageAsCategoryMixin, PlaceholderAdminMixin, PolymorphicChildModelAdmin):
    base_model = Product
    fieldsets = [
        (None, {
            'fields': ['title', 'author', 'genre', 'product_name', 'slug', 'unit_price', 'active', 'manufacturer'],
        }),
        (_("Translatable Fields"), {
            'fields': ['caption', 'description'],
        }),
        (_("Properties"), {
            'fields': [
                #~ 'manufacturer', 'battery_type', 'battery_capacity', 'ram_storage', 'wifi_connectivity', 'bluetooth', 'gps', 'screen_size'
                       ('width', 
                        'height', 
                        #~ 'weight',
                       )],
    ...
    filter_horizontal = ['cms_pages']
    inlines = [ProductImageInline, KhimageInline]
    prepopulated_fields = {'slug': ['product_name'],}
    ...
    def save_model(self, request, obj, form, change):
        ...
        super(KhimageAdmin, self).save_model(request, obj, form, change)
...
class ScooterInline(admin.TabularInline):
    model = ScooterVariant
    extra = 0


@admin.register(ScooterModel)
class ScooterAdmin(InvalidateProductCacheMixin, SortableAdminMixin, TranslatableAdmin, FrontendEditableAdminMixin,
                      CMSPageAsCategoryMixin, PlaceholderAdminMixin, PolymorphicChildModelAdmin):
    base_model = Product
    fieldsets = [
        (None, {
        ...
        (_("Properties"), {
            'fields': ['manufacturer', 'battery_type', 'battery_capacity', 
                        #~ 'ram_storage', 'wifi_connectivity', 
                        'bluetooth', 'operating_system', 'max_speed', 'mileage', 'power',
                       ('width', 'height', 'depth',)
                       , 'weight'
                       ],
        }),
    ...
    filter_horizontal = ['cms_pages']
    inlines = [ProductImageInline, ScooterInline]
    prepopulated_fields = {'slug': ['product_name'],}
    ...
    def save_model(self, request, obj, form, change):
        if not change:
            ...
            max_order = self.base_model.objects.aggregate(max_order=Max('order'))['max_order']
            obj.order = max_order + 1 if max_order else 1
        super(ScooterAdmin, self).save_model(request, obj, form, change)
...
class ProductAdmin(PolymorphicSortableAdminMixin, PolymorphicParentModelAdmin):
    base_model = Product
    child_models = [ScooterModel, KhimageModel, SmartPhoneModel, SmartCard, Commodity]
